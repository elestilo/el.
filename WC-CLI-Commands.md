<!-- START doctoc generated TOC please keep comment here to allow auto update -->
<!-- DON'T EDIT THIS SECTION, INSTEAD RE-RUN doctoc TO UPDATE -->
## Table of Contents

- [Commands](#commands)
  - [wc shop_coupon](#wc-shop_coupon)
    - [wc shop_coupon list](#wc-shop_coupon-list)
      - [--context](#--context)
      - [--page](#--page)
      - [--per_page](#--per_page)
      - [--search](#--search)
      - [--after](#--after)
      - [--before](#--before)
      - [--exclude](#--exclude)
      - [--include](#--include)
      - [--offset](#--offset)
      - [--order](#--order)
      - [--orderby](#--orderby)
      - [--filter](#--filter)
      - [--code](#--code)
      - [--fields](#--fields)
      - [--field](#--field)
      - [--format](#--format)
    - [wc shop_coupon create](#wc-shop_coupon-create)
      - [--code](#--code-1)
      - [--description](#--description)
      - [--discount_type](#--discount_type)
      - [--amount](#--amount)
      - [--expiry_date](#--expiry_date)
      - [--individual_use](#--individual_use)
      - [--product_ids](#--product_ids)
      - [--exclude_product_ids](#--exclude_product_ids)
      - [--usage_limit](#--usage_limit)
      - [--usage_limit_per_user](#--usage_limit_per_user)
      - [--limit_usage_to_x_items](#--limit_usage_to_x_items)
      - [--free_shipping](#--free_shipping)
      - [--product_categories](#--product_categories)
      - [--excluded_product_categories](#--excluded_product_categories)
      - [--exclude_sale_items](#--exclude_sale_items)
      - [--minimum_amount](#--minimum_amount)
      - [--maximum_amount](#--maximum_amount)
      - [--email_restrictions](#--email_restrictions)
      - [--meta_data](#--meta_data)
      - [--porcelain](#--porcelain)
    - [wc shop_coupon get <id>](#wc-shop_coupon-get-id)
      - [--context](#--context-1)
      - [--field](#--field-1)
      - [--fields](#--fields-1)
      - [--format](#--format-1)
    - [wc shop_coupon update <id>](#wc-shop_coupon-update-id)
      - [--code](#--code-2)
      - [--description](#--description-1)
      - [--discount_type](#--discount_type-1)
      - [--amount](#--amount-1)
      - [--expiry_date](#--expiry_date-1)
      - [--individual_use](#--individual_use-1)
      - [--product_ids](#--product_ids-1)
      - [--exclude_product_ids](#--exclude_product_ids-1)
      - [--usage_limit](#--usage_limit-1)
      - [--usage_limit_per_user](#--usage_limit_per_user-1)
      - [--limit_usage_to_x_items](#--limit_usage_to_x_items-1)
      - [--free_shipping](#--free_shipping-1)
      - [--product_categories](#--product_categories-1)
      - [--excluded_product_categories](#--excluded_product_categories-1)
      - [--exclude_sale_items](#--exclude_sale_items-1)
      - [--minimum_amount](#--minimum_amount-1)
      - [--maximum_amount](#--maximum_amount-1)
      - [--email_restrictions](#--email_restrictions-1)
      - [--meta_data](#--meta_data-1)
      - [--porcelain](#--porcelain-1)
    - [wc shop_coupon delete <id>](#wc-shop_coupon-delete-id)
      - [--force](#--force)
      - [--porcelain](#--porcelain-2)
  - [wc customer_download](#wc-customer_download)
    - [wc customer_download list <customer_id>](#wc-customer_download-list-customer_id)
      - [--context](#--context-2)
      - [--field](#--field-2)
      - [--fields](#--fields-2)
      - [--format](#--format-2)
  - [wc customer](#wc-customer)
    - [wc customer list](#wc-customer-list)
      - [--context](#--context-3)
      - [--page](#--page-1)
      - [--per_page](#--per_page-1)
      - [--search](#--search-1)
      - [--exclude](#--exclude-1)
      - [--include](#--include-1)
      - [--offset](#--offset-1)
      - [--order](#--order-1)
      - [--orderby](#--orderby-1)
      - [--email](#--email)
      - [--role](#--role)
      - [--field](#--field-3)
      - [--fields](#--fields-3)
      - [--format](#--format-3)
    - [wc customer create](#wc-customer-create)
      - [--email](#--email-1)
      - [--first_name](#--first_name)
      - [--last_name](#--last_name)
      - [--username](#--username)
      - [--password](#--password)
      - [--billing](#--billing)
      - [--shipping](#--shipping)
      - [--meta_data](#--meta_data-2)
      - [--porcelain](#--porcelain-3)
    - [wc customer get <id>](#wc-customer-get-id)
      - [--context](#--context-4)
      - [--field](#--field-4)
      - [--fields](#--fields-4)
      - [--format](#--format-4)
    - [wc customer update <id>](#wc-customer-update-id)
      - [--email](#--email-2)
      - [--first_name](#--first_name-1)
      - [--last_name](#--last_name-1)
      - [--username](#--username-1)
      - [--password](#--password-1)
      - [--billing](#--billing-1)
      - [--shipping](#--shipping-1)
      - [--meta_data](#--meta_data-3)
      - [--porcelain](#--porcelain-4)
    - [wc customer delete <id>](#wc-customer-delete-id)
      - [--force](#--force-1)
      - [--reassign](#--reassign)
      - [--porcelain](#--porcelain-5)
  - [wc order_note](#wc-order_note)
    - [wc order_note list <order_id>](#wc-order_note-list-order_id)
      - [--context](#--context-5)
      - [--type](#--type)
      - [--field](#--field-5)
      - [--fields](#--fields-5)
      - [--format](#--format-5)
    - [wc order_note create <order_id>](#wc-order_note-create-order_id)
      - [--note](#--note)
      - [--customer_note](#--customer_note)
      - [--porcelain](#--porcelain-6)
    - [wc order_note get <order_id> <id>](#wc-order_note-get-order_id-id)
      - [--context](#--context-6)
      - [--fields](#--fields-6)
      - [--field](#--field-6)
      - [--format](#--format-6)
    - [wc order_note delete <order_id> <id>](#wc-order_note-delete-order_id-id)
      - [--force](#--force-2)
      - [--porcelain](#--porcelain-7)
  - [wc shop_order_refund](#wc-shop_order_refund)
    - [wc shop_order_refund list <order_id>](#wc-shop_order_refund-list-order_id)
      - [--context](#--context-7)
      - [--page](#--page-2)
      - [--per_page](#--per_page-2)
      - [--search](#--search-2)
      - [--after](#--after-1)
      - [--before](#--before-1)
      - [--exclude](#--exclude-2)
      - [--include](#--include-2)
      - [--offset](#--offset-2)
      - [--order](#--order-2)
      - [--orderby](#--orderby-2)
      - [--filter](#--filter-1)
      - [--status](#--status)
      - [--customer](#--customer)
      - [--product](#--product)
      - [--dp](#--dp)
      - [--field](#--field-7)
      - [--fields](#--fields-7)
      - [--format](#--format-7)
    - [wc shop_order_refund create <order_id>](#wc-shop_order_refund-create-order_id)
      - [--amount](#--amount-2)
      - [--reason](#--reason)
      - [--refunded_by](#--refunded_by)
      - [--meta_data](#--meta_data-4)
      - [--line_items](#--line_items)
      - [--porcelain](#--porcelain-8)
    - [wc shop_order_refund get <order_id> <id>](#wc-shop_order_refund-get-order_id-id)
      - [--context](#--context-8)
      - [--fields](#--fields-8)
      - [--field](#--field-8)
      - [--format](#--format-8)
    - [wc shop_order_refund delete <order_id> <id>](#wc-shop_order_refund-delete-order_id-id)
      - [--force](#--force-3)
      - [--porcelain](#--porcelain-9)
  - [wc shop_order](#wc-shop_order)
    - [wc shop_order list](#wc-shop_order-list)
      - [--context](#--context-9)
      - [--page](#--page-3)
      - [--per_page](#--per_page-3)
      - [--search](#--search-3)
      - [--after](#--after-2)
      - [--before](#--before-2)
      - [--exclude](#--exclude-3)
      - [--include](#--include-3)
      - [--offset](#--offset-3)
      - [--order](#--order-3)
      - [--orderby](#--orderby-3)
      - [--filter](#--filter-2)
      - [--status](#--status-1)
      - [--customer](#--customer-1)
      - [--product](#--product-1)
      - [--dp](#--dp-1)
      - [--fields](#--fields-9)
      - [--field](#--field-9)
      - [--format](#--format-9)
    - [wc shop_order create](#wc-shop_order-create)
      - [--parent_id](#--parent_id)
      - [--status](#--status-2)
      - [--currency](#--currency)
      - [--date_created](#--date_created)
      - [--customer_id](#--customer_id)
      - [--billing](#--billing-2)
      - [--shipping](#--shipping-2)
      - [--payment_method](#--payment_method)
      - [--payment_method_title](#--payment_method_title)
      - [--transaction_id](#--transaction_id)
      - [--customer_note](#--customer_note-1)
      - [--date_completed](#--date_completed)
      - [--date_paid](#--date_paid)
      - [--meta_data](#--meta_data-5)
      - [--line_items](#--line_items-1)
      - [--shipping_lines](#--shipping_lines)
      - [--fee_lines](#--fee_lines)
      - [--coupon_lines](#--coupon_lines)
      - [--set_paid](#--set_paid)
      - [--porcelain](#--porcelain-10)
    - [wc shop_order get <id>](#wc-shop_order-get-id)
      - [--context](#--context-10)
      - [--field](#--field-10)
      - [--fields](#--fields-10)
      - [--format](#--format-10)
    - [wc shop_order update <id>](#wc-shop_order-update-id)
      - [--parent_id](#--parent_id-1)
      - [--status](#--status-3)
      - [--currency](#--currency-1)
      - [--date_created](#--date_created-1)
      - [--customer_id](#--customer_id-1)
      - [--billing](#--billing-3)
      - [--shipping](#--shipping-3)
      - [--payment_method](#--payment_method-1)
      - [--payment_method_title](#--payment_method_title-1)
      - [--transaction_id](#--transaction_id-1)
      - [--customer_note](#--customer_note-2)
      - [--date_completed](#--date_completed-1)
      - [--date_paid](#--date_paid-1)
      - [--meta_data](#--meta_data-6)
      - [--line_items](#--line_items-2)
      - [--shipping_lines](#--shipping_lines-1)
      - [--fee_lines](#--fee_lines-1)
      - [--coupon_lines](#--coupon_lines-1)
      - [--set_paid](#--set_paid-1)
      - [--porcelain](#--porcelain-11)
    - [wc shop_order delete <id>](#wc-shop_order-delete-id)
      - [--force](#--force-4)
      - [--porcelain](#--porcelain-12)
  - [wc product_attribute_term](#wc-product_attribute_term)
    - [wc product_attribute_term list <attribute_id>](#wc-product_attribute_term-list-attribute_id)
      - [--context](#--context-11)
      - [--page](#--page-4)
      - [--per_page](#--per_page-4)
      - [--search](#--search-4)
      - [--exclude](#--exclude-4)
      - [--include](#--include-4)
      - [--order](#--order-4)
      - [--orderby](#--orderby-4)
      - [--hide_empty](#--hide_empty)
      - [--parent](#--parent)
      - [--product](#--product-2)
      - [--slug](#--slug)
      - [--fields](#--fields-11)
      - [--field](#--field-11)
      - [--format](#--format-11)
    - [wc product_attribute_term create <attribute_id>](#wc-product_attribute_term-create-attribute_id)
      - [--name](#--name)
      - [--slug](#--slug-1)
      - [--description](#--description-2)
      - [--menu_order](#--menu_order)
      - [--porcelain](#--porcelain-13)
    - [wc product_attribute_term get <attribute_id> <id>](#wc-product_attribute_term-get-attribute_id-id)
      - [--context](#--context-12)
      - [--fields](#--fields-12)
      - [--field](#--field-12)
      - [--format](#--format-12)
    - [wc product_attribute_term update <attribute_id> <id>](#wc-product_attribute_term-update-attribute_id-id)
      - [--name](#--name-1)
      - [--slug](#--slug-2)
      - [--description](#--description-3)
      - [--menu_order](#--menu_order-1)
      - [--porcelain](#--porcelain-14)
    - [wc product_attribute_term delete <attribute_id> <id>](#wc-product_attribute_term-delete-attribute_id-id)
      - [--force](#--force-5)
      - [--porcelain](#--porcelain-15)
  - [wc product_attribute](#wc-product_attribute)
    - [wc product_attribute list](#wc-product_attribute-list)
      - [--context](#--context-13)
      - [--field](#--field-13)
      - [--fields](#--fields-13)
      - [--format](#--format-13)
    - [wc product_attribute create](#wc-product_attribute-create)
      - [--name](#--name-2)
      - [--slug](#--slug-3)
      - [--type](#--type-1)
      - [--order_by](#--order_by)
      - [--has_archives](#--has_archives)
      - [--porcelain](#--porcelain-16)
    - [wc product_attribute get <id>](#wc-product_attribute-get-id)
      - [--context](#--context-14)
      - [--fields](#--fields-14)
      - [--field](#--field-14)
      - [--format](#--format-14)
    - [wc product_attribute update <id>](#wc-product_attribute-update-id)
      - [--name](#--name-3)
      - [--slug](#--slug-4)
      - [--type](#--type-2)
      - [--order_by](#--order_by-1)
      - [--has_archives](#--has_archives-1)
      - [--porcelain](#--porcelain-17)
    - [wc product_attribute delete <id>](#wc-product_attribute-delete-id)
      - [--force](#--force-6)
      - [--porcelain](#--porcelain-18)
  - [wc product_cat](#wc-product_cat)
    - [wc product_cat list](#wc-product_cat-list)
      - [--context](#--context-15)
      - [--page](#--page-5)
      - [--per_page](#--per_page-5)
      - [--search](#--search-5)
      - [--exclude](#--exclude-5)
      - [--include](#--include-5)
      - [--order](#--order-5)
      - [--orderby](#--orderby-5)
      - [--hide_empty](#--hide_empty-1)
      - [--parent](#--parent-1)
      - [--product](#--product-3)
      - [--slug](#--slug-5)
      - [--field](#--field-15)
      - [--fields](#--fields-15)
      - [--format](#--format-15)
    - [wc product_cat create](#wc-product_cat-create)
      - [--name](#--name-4)
      - [--slug](#--slug-6)
      - [--parent](#--parent-2)
      - [--description](#--description-4)
      - [--display](#--display)
      - [--image](#--image)
      - [--menu_order](#--menu_order-2)
      - [--porcelain](#--porcelain-19)
    - [wc product_cat get <id>](#wc-product_cat-get-id)
      - [--context](#--context-16)
      - [--fields](#--fields-16)
      - [--field](#--field-16)
      - [--format](#--format-16)
    - [wc product_cat update <id>](#wc-product_cat-update-id)
      - [--name](#--name-5)
      - [--slug](#--slug-7)
      - [--parent](#--parent-3)
      - [--description](#--description-5)
      - [--display](#--display-1)
      - [--image](#--image-1)
      - [--menu_order](#--menu_order-3)
      - [--porcelain](#--porcelain-20)
    - [wc product_cat delete <id>](#wc-product_cat-delete-id)
      - [--force](#--force-7)
      - [--porcelain](#--porcelain-21)
  - [wc product_review](#wc-product_review)
    - [wc product_review list <product_id>](#wc-product_review-list-product_id)
      - [--context](#--context-17)
      - [--fields](#--fields-17)
      - [--field](#--field-17)
      - [--format](#--format-17)
    - [wc product_review create <product_id>](#wc-product_review-create-product_id)
      - [--review](#--review)
      - [--date_created](#--date_created-2)
      - [--rating](#--rating)
      - [--name](#--name-6)
      - [--email](#--email-3)
      - [--porcelain](#--porcelain-22)
    - [wc product_review get <product_id> <id>](#wc-product_review-get-product_id-id)
      - [--context](#--context-18)
      - [--fields](#--fields-18)
      - [--field](#--field-18)
      - [--format](#--format-18)
    - [wc product_review update <product_id> <id>](#wc-product_review-update-product_id-id)
      - [--review](#--review-1)
      - [--date_created](#--date_created-3)
      - [--rating](#--rating-1)
      - [--name](#--name-7)
      - [--email](#--email-4)
      - [--porcelain](#--porcelain-23)
    - [wc product_review delete <product_id> <id>](#wc-product_review-delete-product_id-id)
      - [--force](#--force-8)
      - [--porcelain](#--porcelain-24)
  - [wc product_shipping_class](#wc-product_shipping_class)
    - [wc product_shipping_class list](#wc-product_shipping_class-list)
      - [--context](#--context-19)
      - [--page](#--page-6)
      - [--per_page](#--per_page-6)
      - [--search](#--search-6)
      - [--exclude](#--exclude-6)
      - [--include](#--include-6)
      - [--offset](#--offset-4)
      - [--order](#--order-6)
      - [--orderby](#--orderby-6)
      - [--hide_empty](#--hide_empty-2)
      - [--product](#--product-4)
      - [--slug](#--slug-8)
      - [--field](#--field-19)
      - [--fields](#--fields-19)
      - [--format](#--format-19)
    - [wc product_shipping_class create](#wc-product_shipping_class-create)
      - [--name](#--name-8)
      - [--slug](#--slug-9)
      - [--description](#--description-6)
      - [--porcelain](#--porcelain-25)
    - [wc product_shipping_class get <id>](#wc-product_shipping_class-get-id)
      - [--context](#--context-20)
      - [--field](#--field-20)
      - [--fields](#--fields-20)
      - [--format](#--format-20)
    - [wc product_shipping_class update <id>](#wc-product_shipping_class-update-id)
      - [--name](#--name-9)
      - [--slug](#--slug-10)
      - [--description](#--description-7)
      - [--porcelain](#--porcelain-26)
    - [wc product_shipping_class delete <id>](#wc-product_shipping_class-delete-id)
      - [--force](#--force-9)
      - [--porcelain](#--porcelain-27)
  - [wc product_tag](#wc-product_tag)
    - [wc product_tag list](#wc-product_tag-list)
      - [--context](#--context-21)
      - [--page](#--page-7)
      - [--per_page](#--per_page-7)
      - [--search](#--search-7)
      - [--exclude](#--exclude-7)
      - [--include](#--include-7)
      - [--offset](#--offset-5)
      - [--order](#--order-7)
      - [--orderby](#--orderby-7)
      - [--hide_empty](#--hide_empty-3)
      - [--product](#--product-5)
      - [--slug](#--slug-11)
      - [--fields](#--fields-21)
      - [--field](#--field-21)
      - [--format](#--format-21)
    - [wc product_tag create](#wc-product_tag-create)
      - [--name](#--name-10)
      - [--slug](#--slug-12)
      - [--description](#--description-8)
      - [--porcelain](#--porcelain-28)
    - [wc product_tag get <id>](#wc-product_tag-get-id)
      - [--context](#--context-22)
      - [--field](#--field-22)
      - [--fields](#--fields-22)
      - [--format](#--format-22)
    - [wc product_tag update <id>](#wc-product_tag-update-id)
      - [--name](#--name-11)
      - [--slug](#--slug-13)
      - [--description](#--description-9)
      - [--porcelain](#--porcelain-29)
    - [wc product_tag delete <id>](#wc-product_tag-delete-id)
      - [--force](#--force-10)
      - [--porcelain](#--porcelain-30)
  - [wc product](#wc-product)
    - [wc product list](#wc-product-list)
      - [--context](#--context-23)
      - [--page](#--page-8)
      - [--per_page](#--per_page-8)
      - [--search](#--search-8)
      - [--after](#--after-3)
      - [--before](#--before-3)
      - [--exclude](#--exclude-8)
      - [--include](#--include-8)
      - [--offset](#--offset-6)
      - [--order](#--order-8)
      - [--orderby](#--orderby-8)
      - [--filter](#--filter-3)
      - [--slug](#--slug-14)
      - [--status](#--status-4)
      - [--type](#--type-3)
      - [--sku](#--sku)
      - [--featured](#--featured)
      - [--category](#--category)
      - [--tag](#--tag)
      - [--shipping_class](#--shipping_class)
      - [--attribute](#--attribute)
      - [--attribute_term](#--attribute_term)
      - [--tax_class](#--tax_class)
      - [--in_stock](#--in_stock)
      - [--on_sale](#--on_sale)
      - [--min_price](#--min_price)
      - [--max_price](#--max_price)
      - [--fields](#--fields-23)
      - [--field](#--field-23)
      - [--format](#--format-23)
    - [wc product create](#wc-product-create)
      - [--name](#--name-12)
      - [--slug](#--slug-15)
      - [--type](#--type-4)
      - [--status](#--status-5)
      - [--featured](#--featured-1)
      - [--catalog_visibility](#--catalog_visibility)
      - [--description](#--description-10)
      - [--short_description](#--short_description)
      - [--sku](#--sku-1)
      - [--regular_price](#--regular_price)
      - [--sale_price](#--sale_price)
      - [--date_on_sale_from](#--date_on_sale_from)
      - [--date_on_sale_to](#--date_on_sale_to)
      - [--virtual](#--virtual)
      - [--downloadable](#--downloadable)
      - [--downloads](#--downloads)
      - [--download_limit](#--download_limit)
      - [--download_expiry](#--download_expiry)
      - [--download_type](#--download_type)
      - [--external_url](#--external_url)
      - [--button_text](#--button_text)
      - [--tax_status](#--tax_status)
      - [--tax_class](#--tax_class-1)
      - [--manage_stock](#--manage_stock)
      - [--stock_quantity](#--stock_quantity)
      - [--in_stock](#--in_stock-1)
      - [--backorders](#--backorders)
      - [--sold_individually](#--sold_individually)
      - [--weight](#--weight)
      - [--dimensions](#--dimensions)
      - [--shipping_class](#--shipping_class-1)
      - [--reviews_allowed](#--reviews_allowed)
      - [--upsell_ids](#--upsell_ids)
      - [--cross_sell_ids](#--cross_sell_ids)
      - [--parent_id](#--parent_id-2)
      - [--purchase_note](#--purchase_note)
      - [--categories](#--categories)
      - [--tags](#--tags)
      - [--images](#--images)
      - [--attributes](#--attributes)
      - [--default_attributes](#--default_attributes)
      - [--variations](#--variations)
      - [--menu_order](#--menu_order-4)
      - [--porcelain](#--porcelain-31)
    - [wc product get <id>](#wc-product-get-id)
      - [--context](#--context-24)
      - [--field](#--field-24)
      - [--fields](#--fields-24)
      - [--format](#--format-24)
    - [wc product update <id>](#wc-product-update-id)
      - [--name](#--name-13)
      - [--slug](#--slug-16)
      - [--type](#--type-5)
      - [--status](#--status-6)
      - [--featured](#--featured-2)
      - [--catalog_visibility](#--catalog_visibility-1)
      - [--description](#--description-11)
      - [--short_description](#--short_description-1)
      - [--sku](#--sku-2)
      - [--regular_price](#--regular_price-1)
      - [--sale_price](#--sale_price-1)
      - [--date_on_sale_from](#--date_on_sale_from-1)
      - [--date_on_sale_to](#--date_on_sale_to-1)
      - [--virtual](#--virtual-1)
      - [--downloadable](#--downloadable-1)
      - [--downloads](#--downloads-1)
      - [--download_limit](#--download_limit-1)
      - [--download_expiry](#--download_expiry-1)
      - [--download_type](#--download_type-1)
      - [--external_url](#--external_url-1)
      - [--button_text](#--button_text-1)
      - [--tax_status](#--tax_status-1)
      - [--tax_class](#--tax_class-2)
      - [--manage_stock](#--manage_stock-1)
      - [--stock_quantity](#--stock_quantity-1)
      - [--in_stock](#--in_stock-2)
      - [--backorders](#--backorders-1)
      - [--sold_individually](#--sold_individually-1)
      - [--weight](#--weight-1)
      - [--dimensions](#--dimensions-1)
      - [--shipping_class](#--shipping_class-2)
      - [--reviews_allowed](#--reviews_allowed-1)
      - [--upsell_ids](#--upsell_ids-1)
      - [--cross_sell_ids](#--cross_sell_ids-1)
      - [--parent_id](#--parent_id-3)
      - [--purchase_note](#--purchase_note-1)
      - [--categories](#--categories-1)
      - [--tags](#--tags-1)
      - [--images](#--images-1)
      - [--attributes](#--attributes-1)
      - [--default_attributes](#--default_attributes-1)
      - [--variations](#--variations-1)
      - [--menu_order](#--menu_order-5)
      - [--porcelain](#--porcelain-32)
    - [wc product delete <id>](#wc-product-delete-id)
      - [--force](#--force-11)
      - [--porcelain](#--porcelain-33)
  - [wc product_variation](#wc-product_variation)
    - [wc product_variation list <product_id>](#wc-product_variation-list-product_id)
      - [--context](#--context-25)
      - [--page](#--page-9)
      - [--per_page](#--per_page-9)
      - [--search](#--search-9)
      - [--after](#--after-4)
      - [--before](#--before-4)
      - [--exclude](#--exclude-9)
      - [--include](#--include-9)
      - [--offset](#--offset-7)
      - [--order](#--order-9)
      - [--orderby](#--orderby-9)
      - [--filter](#--filter-4)
      - [--slug](#--slug-17)
      - [--status](#--status-7)
      - [--type](#--type-6)
      - [--sku](#--sku-3)
      - [--featured](#--featured-3)
      - [--category](#--category-1)
      - [--tag](#--tag-1)
      - [--shipping_class](#--shipping_class-3)
      - [--attribute](#--attribute-1)
      - [--attribute_term](#--attribute_term-1)
      - [--tax_class](#--tax_class-3)
      - [--in_stock](#--in_stock-3)
      - [--on_sale](#--on_sale-1)
      - [--min_price](#--min_price-1)
      - [--max_price](#--max_price-1)
      - [--fields](#--fields-25)
      - [--field](#--field-25)
      - [--format](#--format-25)
    - [wc product_variation create <product_id>](#wc-product_variation-create-product_id)
      - [--description](#--description-12)
      - [--sku](#--sku-4)
      - [--regular_price](#--regular_price-2)
      - [--sale_price](#--sale_price-2)
      - [--date_on_sale_from](#--date_on_sale_from-2)
      - [--date_on_sale_to](#--date_on_sale_to-2)
      - [--visible](#--visible)
      - [--virtual](#--virtual-2)
      - [--downloadable](#--downloadable-2)
      - [--downloads](#--downloads-2)
      - [--download_limit](#--download_limit-2)
      - [--download_expiry](#--download_expiry-2)
      - [--tax_status](#--tax_status-2)
      - [--tax_class](#--tax_class-4)
      - [--manage_stock](#--manage_stock-2)
      - [--stock_quantity](#--stock_quantity-2)
      - [--in_stock](#--in_stock-4)
      - [--backorders](#--backorders-2)
      - [--weight](#--weight-2)
      - [--dimensions](#--dimensions-2)
      - [--shipping_class](#--shipping_class-4)
      - [--image](#--image-2)
      - [--attributes](#--attributes-2)
      - [--porcelain](#--porcelain-34)
    - [wc product_variation get <product_id> <id>](#wc-product_variation-get-product_id-id)
      - [--context](#--context-26)
      - [--fields](#--fields-26)
      - [--field](#--field-26)
      - [--format](#--format-26)
    - [wc product_variation update <product_id> <id>](#wc-product_variation-update-product_id-id)
      - [--description](#--description-13)
      - [--sku](#--sku-5)
      - [--regular_price](#--regular_price-3)
      - [--sale_price](#--sale_price-3)
      - [--date_on_sale_from](#--date_on_sale_from-3)
      - [--date_on_sale_to](#--date_on_sale_to-3)
      - [--visible](#--visible-1)
      - [--virtual](#--virtual-3)
      - [--downloadable](#--downloadable-3)
      - [--downloads](#--downloads-3)
      - [--download_limit](#--download_limit-3)
      - [--download_expiry](#--download_expiry-3)
      - [--tax_status](#--tax_status-3)
      - [--tax_class](#--tax_class-5)
      - [--manage_stock](#--manage_stock-3)
      - [--stock_quantity](#--stock_quantity-3)
      - [--in_stock](#--in_stock-5)
      - [--backorders](#--backorders-3)
      - [--weight](#--weight-3)
      - [--dimensions](#--dimensions-3)
      - [--shipping_class](#--shipping_class-5)
      - [--image](#--image-3)
      - [--attributes](#--attributes-3)
      - [--porcelain](#--porcelain-35)
    - [wc product_variation delete <product_id> <id>](#wc-product_variation-delete-product_id-id)
      - [--force](#--force-12)
      - [--porcelain](#--porcelain-36)
  - [wc shipping_zone](#wc-shipping_zone)
    - [wc shipping_zone list](#wc-shipping_zone-list)
      - [--fields](#--fields-27)
      - [--field](#--field-27)
      - [--format](#--format-27)
    - [wc shipping_zone create](#wc-shipping_zone-create)
      - [--name](#--name-14)
      - [--order](#--order-10)
      - [--porcelain](#--porcelain-37)
    - [wc shipping_zone get <id>](#wc-shipping_zone-get-id)
      - [--field](#--field-28)
      - [--fields](#--fields-28)
      - [--format](#--format-28)
    - [wc shipping_zone update <id>](#wc-shipping_zone-update-id)
      - [--name](#--name-15)
      - [--order](#--order-11)
      - [--porcelain](#--porcelain-38)
    - [wc shipping_zone delete <id>](#wc-shipping_zone-delete-id)
      - [--force](#--force-13)
      - [--porcelain](#--porcelain-39)
  - [wc shipping_zone_location](#wc-shipping_zone_location)
    - [wc shipping_zone_location list](#wc-shipping_zone_location-list)
      - [--fields](#--fields-29)
      - [--field](#--field-29)
      - [--format](#--format-29)
  - [wc shipping_zone_method](#wc-shipping_zone_method)
    - [wc shipping_zone_method list](#wc-shipping_zone_method-list)
      - [--fields](#--fields-30)
      - [--field](#--field-30)
      - [--format](#--format-30)
    - [wc shipping_zone_method create](#wc-shipping_zone_method-create)
      - [--order](#--order-12)
      - [--enabled](#--enabled)
      - [--method_id](#--method_id)
      - [--settings](#--settings)
      - [--porcelain](#--porcelain-40)
    - [wc shipping_zone_method get <id>](#wc-shipping_zone_method-get-id)
      - [--fields](#--fields-31)
      - [--field](#--field-31)
      - [--format](#--format-31)
    - [wc shipping_zone_method update <id>](#wc-shipping_zone_method-update-id)
      - [--order](#--order-13)
      - [--enabled](#--enabled-1)
      - [--method_id](#--method_id-1)
      - [--settings](#--settings-1)
      - [--porcelain](#--porcelain-41)
    - [wc shipping_zone_method delete <id>](#wc-shipping_zone_method-delete-id)
      - [--force](#--force-14)
      - [--porcelain](#--porcelain-42)
  - [wc tax_class](#wc-tax_class)
    - [wc tax_class list](#wc-tax_class-list)
      - [--context](#--context-27)
      - [--fields](#--fields-32)
      - [--field](#--field-32)
      - [--format](#--format-32)
    - [wc tax_class create](#wc-tax_class-create)
      - [--name](#--name-16)
      - [--porcelain](#--porcelain-43)
    - [wc tax_class delete <id>](#wc-tax_class-delete-id)
      - [--force](#--force-15)
      - [--porcelain](#--porcelain-44)
  - [wc tax](#wc-tax)
    - [wc tax list](#wc-tax-list)
      - [--context](#--context-28)
      - [--page](#--page-10)
      - [--per_page](#--per_page-10)
      - [--search](#--search-10)
      - [--exclude](#--exclude-10)
      - [--include](#--include-10)
      - [--offset](#--offset-8)
      - [--order](#--order-14)
      - [--orderby](#--orderby-10)
      - [--class](#--class)
      - [--fields](#--fields-33)
      - [--field](#--field-33)
      - [--format](#--format-33)
    - [wc tax create](#wc-tax-create)
      - [--country](#--country)
      - [--state](#--state)
      - [--postcode](#--postcode)
      - [--city](#--city)
      - [--rate](#--rate)
      - [--name](#--name-17)
      - [--priority](#--priority)
      - [--compound](#--compound)
      - [--shipping](#--shipping-4)
      - [--order](#--order-15)
      - [--class](#--class-1)
      - [--porcelain](#--porcelain-45)
    - [wc tax get <id>](#wc-tax-get-id)
      - [--context](#--context-29)
      - [--fields](#--fields-34)
      - [--field](#--field-34)
      - [--format](#--format-34)
    - [wc tax update <id>](#wc-tax-update-id)
      - [--country](#--country-1)
      - [--state](#--state-1)
      - [--postcode](#--postcode-1)
      - [--city](#--city-1)
      - [--rate](#--rate-1)
      - [--name](#--name-18)
      - [--priority](#--priority-1)
      - [--compound](#--compound-1)
      - [--shipping](#--shipping-5)
      - [--order](#--order-16)
      - [--class](#--class-2)
      - [--porcelain](#--porcelain-46)
    - [wc tax delete <id>](#wc-tax-delete-id)
      - [--force](#--force-16)
      - [--porcelain](#--porcelain-47)
  - [wc webhook_delivery](#wc-webhook_delivery)
    - [wc webhook_delivery list](#wc-webhook_delivery-list)
      - [--context](#--context-30)
      - [--field](#--field-35)
      - [--fields](#--fields-35)
      - [--format](#--format-35)
    - [wc webhook_delivery get <id>](#wc-webhook_delivery-get-id)
      - [--context](#--context-31)
      - [--fields](#--fields-36)
      - [--field](#--field-36)
      - [--format](#--format-36)
  - [wc webhook](#wc-webhook)
    - [wc webhook list](#wc-webhook-list)
      - [--context](#--context-32)
      - [--page](#--page-11)
      - [--per_page](#--per_page-11)
      - [--search](#--search-11)
      - [--after](#--after-5)
      - [--before](#--before-5)
      - [--exclude](#--exclude-11)
      - [--include](#--include-11)
      - [--offset](#--offset-9)
      - [--order](#--order-17)
      - [--orderby](#--orderby-11)
      - [--filter](#--filter-5)
      - [--status](#--status-8)
      - [--fields](#--fields-37)
      - [--field](#--field-37)
      - [--format](#--format-37)
    - [wc webhook create](#wc-webhook-create)
      - [--name](#--name-19)
      - [--status](#--status-9)
      - [--topic](#--topic)
      - [--secret](#--secret)
      - [--delivery_url](#--delivery_url)
      - [--porcelain](#--porcelain-48)
    - [wc webhook get <id>](#wc-webhook-get-id)
      - [--context](#--context-33)
      - [--fields](#--fields-38)
      - [--field](#--field-38)
      - [--format](#--format-38)
    - [wc webhook update <id>](#wc-webhook-update-id)
      - [--name](#--name-20)
      - [--status](#--status-10)
      - [--topic](#--topic-1)
      - [--secret](#--secret-1)
      - [--porcelain](#--porcelain-49)
    - [wc webhook delete <id>](#wc-webhook-delete-id)
      - [--force](#--force-17)
      - [--porcelain](#--porcelain-50)
  - [wc shipping_method](#wc-shipping_method)
    - [wc shipping_method list](#wc-shipping_method-list)
      - [--context](#--context-34)
      - [--fields](#--fields-39)
      - [--field](#--field-39)
      - [--format](#--format-39)
    - [wc shipping_method get <id>](#wc-shipping_method-get-id)
      - [--context](#--context-35)
      - [--fields](#--fields-40)
      - [--field](#--field-40)
      - [--format](#--format-40)
  - [wc payment_gateway](#wc-payment_gateway)
    - [wc payment_gateway list](#wc-payment_gateway-list)
      - [--context](#--context-36)
      - [--fields](#--fields-41)
      - [--field](#--field-41)
      - [--format](#--format-41)
    - [wc payment_gateway get <id>](#wc-payment_gateway-get-id)
      - [--context](#--context-37)
      - [--field](#--field-42)
      - [--fields](#--fields-42)
      - [--format](#--format-42)
    - [wc payment_gateway update <id>](#wc-payment_gateway-update-id)
      - [--title](#--title)
      - [--description](#--description-14)
      - [--order](#--order-18)
      - [--enabled](#--enabled-2)
      - [--settings](#--settings-2)
      - [--porcelain](#--porcelain-51)

<!-- END doctoc generated TOC please keep comment here to allow auto update -->

<h1>Commands</h1>
<ul><li><a href="#wc-shop_coupon">wc shop_coupon</a></li>
<li><a href="#wc-customer_download">wc customer_download</a></li>
<li><a href="#wc-customer">wc customer</a></li>
<li><a href="#wc-order_note">wc order_note</a></li>
<li><a href="#wc-shop_order_refund">wc shop_order_refund</a></li>
<li><a href="#wc-shop_order">wc shop_order</a></li>
<li><a href="#wc-product_attribute_term">wc product_attribute_term</a></li>
<li><a href="#wc-product_attribute">wc product_attribute</a></li>
<li><a href="#wc-product_cat">wc product_cat</a></li>
<li><a href="#wc-product_review">wc product_review</a></li>
<li><a href="#wc-product_shipping_class">wc product_shipping_class</a></li>
<li><a href="#wc-product_tag">wc product_tag</a></li>
<li><a href="#wc-product">wc product</a></li>
<li><a href="#wc-product_variation">wc product_variation</a></li>
<li><a href="#wc-shipping_zone">wc shipping_zone</a></li>
<li><a href="#wc-shipping_zone_location">wc shipping_zone_location</a></li>
<li><a href="#wc-shipping_zone_method">wc shipping_zone_method</a></li>
<li><a href="#wc-tax_class">wc tax_class</a></li>
<li><a href="#wc-tax">wc tax</a></li>
<li><a href="#wc-webhook_delivery">wc webhook_delivery</a></li>
<li><a href="#wc-webhook">wc webhook</a></li>
<li><a href="#wc-shipping_method">wc shipping_method</a></li>
<li><a href="#wc-payment_gateway">wc payment_gateway</a></li>
</ul>
<h2>wc shop_coupon</h2>
<h3>wc shop_coupon list</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--page</h4>
<p>Current page of the collection.</p>
<h4>--per_page</h4>
<p>Maximum number of items to be returned in result set.</p>
<h4>--search</h4>
<p>Limit results to those matching a string.</p>
<h4>--after</h4>
<p>Limit response to resources published after a given ISO8601 compliant date.</p>
<h4>--before</h4>
<p>Limit response to resources published before a given ISO8601 compliant date.</p>
<h4>--exclude</h4>
<p>Ensure result set excludes specific IDs.</p>
<h4>--include</h4>
<p>Limit result set to specific ids.</p>
<h4>--offset</h4>
<p>Offset the result set by a specific number of items.</p>
<h4>--order</h4>
<p>Order sort attribute ascending or descending.</p>
<h4>--orderby</h4>
<p>Sort collection by object attribute.</p>
<h4>--filter</h4>
<p>Use WP Query arguments to modify the response; private query vars require appropriate authorization.</p>
<h4>--code</h4>
<p>Limit result set to resources with a specific code.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc shop_coupon create</h3>
<h4>--code</h4>
<p>Coupon code. (<strong>Required</strong>)</p>
<h4>--description</h4>
<p>Coupon description.</p>
<h4>--discount_type</h4>
<p>Determines the type of discount that will be applied.</p>
<h4>--amount</h4>
<p>The amount of discount.</p>
<h4>--expiry_date</h4>
<p>UTC DateTime when the coupon expires.</p>
<h4>--individual_use</h4>
<p>Whether coupon can only be used individually.</p>
<h4>--product_ids</h4>
<p>List of product ID's the coupon can be used on.</p>
<h4>--exclude_product_ids</h4>
<p>List of product ID's the coupon cannot be used on.</p>
<h4>--usage_limit</h4>
<p>How many times the coupon can be used.</p>
<h4>--usage_limit_per_user</h4>
<p>How many times the coupon can be used per customer.</p>
<h4>--limit_usage_to_x_items</h4>
<p>Max number of items in the cart the coupon can be applied to.</p>
<h4>--free_shipping</h4>
<p>Define if can be applied for free shipping.</p>
<h4>--product_categories</h4>
<p>List of category ID's the coupon applies to.</p>
<h4>--excluded_product_categories</h4>
<p>List of category ID's the coupon does not apply to.</p>
<h4>--exclude_sale_items</h4>
<p>Define if should not apply when have sale items.</p>
<h4>--minimum_amount</h4>
<p>Minimum order amount that needs to be in the cart before coupon applies.</p>
<h4>--maximum_amount</h4>
<p>Maximum order amount allowed when using the coupon.</p>
<h4>--email_restrictions</h4>
<p>List of email addresses that can use this coupon.</p>
<h4>--meta_data</h4>
<p>Order meta data.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc shop_coupon get &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc shop_coupon update &lt;id&gt;</h3>
<h4>--code</h4>
<p>Coupon code.</p>
<h4>--description</h4>
<p>Coupon description.</p>
<h4>--discount_type</h4>
<p>Determines the type of discount that will be applied.</p>
<h4>--amount</h4>
<p>The amount of discount.</p>
<h4>--expiry_date</h4>
<p>UTC DateTime when the coupon expires.</p>
<h4>--individual_use</h4>
<p>Whether coupon can only be used individually.</p>
<h4>--product_ids</h4>
<p>List of product ID's the coupon can be used on.</p>
<h4>--exclude_product_ids</h4>
<p>List of product ID's the coupon cannot be used on.</p>
<h4>--usage_limit</h4>
<p>How many times the coupon can be used.</p>
<h4>--usage_limit_per_user</h4>
<p>How many times the coupon can be used per customer.</p>
<h4>--limit_usage_to_x_items</h4>
<p>Max number of items in the cart the coupon can be applied to.</p>
<h4>--free_shipping</h4>
<p>Define if can be applied for free shipping.</p>
<h4>--product_categories</h4>
<p>List of category ID's the coupon applies to.</p>
<h4>--excluded_product_categories</h4>
<p>List of category ID's the coupon does not apply to.</p>
<h4>--exclude_sale_items</h4>
<p>Define if should not apply when have sale items.</p>
<h4>--minimum_amount</h4>
<p>Minimum order amount that needs to be in the cart before coupon applies.</p>
<h4>--maximum_amount</h4>
<p>Maximum order amount allowed when using the coupon.</p>
<h4>--email_restrictions</h4>
<p>List of email addresses that can use this coupon.</p>
<h4>--meta_data</h4>
<p>Order meta data.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc shop_coupon delete &lt;id&gt;</h3>
<h4>--force</h4>
<p>Whether to bypass trash and force deletion.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc customer_download</h2>
<h3>wc customer_download list &lt;customer_id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h2>wc customer</h2>
<h3>wc customer list</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--page</h4>
<p>Current page of the collection.</p>
<h4>--per_page</h4>
<p>Maximum number of items to be returned in result set.</p>
<h4>--search</h4>
<p>Limit results to those matching a string.</p>
<h4>--exclude</h4>
<p>Ensure result set excludes specific IDs.</p>
<h4>--include</h4>
<p>Limit result set to specific IDs.</p>
<h4>--offset</h4>
<p>Offset the result set by a specific number of items.</p>
<h4>--order</h4>
<p>Order sort attribute ascending or descending.</p>
<h4>--orderby</h4>
<p>Sort collection by object attribute.</p>
<h4>--email</h4>
<p>Limit result set to resources with a specific email.</p>
<h4>--role</h4>
<p>Limit result set to resources with a specific role.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc customer create</h3>
<h4>--email</h4>
<p>New user email address. (<strong>Required</strong>)</p>
<h4>--first_name</h4>
<p>Customer first name.</p>
<h4>--last_name</h4>
<p>Customer last name.</p>
<h4>--username</h4>
<p>New user username.</p>
<h4>--password</h4>
<p>New user password. (<strong>Required</strong>)</p>
<h4>--billing</h4>
<p>List of billing address data.</p>
<h4>--shipping</h4>
<p>List of shipping address data.</p>
<h4>--meta_data</h4>
<p>Order meta data.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc customer get &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc customer update &lt;id&gt;</h3>
<h4>--email</h4>
<p>The email address for the customer.</p>
<h4>--first_name</h4>
<p>Customer first name.</p>
<h4>--last_name</h4>
<p>Customer last name.</p>
<h4>--username</h4>
<p>Customer login name.</p>
<h4>--password</h4>
<p>Customer password.</p>
<h4>--billing</h4>
<p>List of billing address data.</p>
<h4>--shipping</h4>
<p>List of shipping address data.</p>
<h4>--meta_data</h4>
<p>Order meta data.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc customer delete &lt;id&gt;</h3>
<h4>--force</h4>
<p>Required to be true, as resource does not support trashing.</p>
<h4>--reassign</h4>
<p>ID to reassign posts to.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc order_note</h2>
<h3>wc order_note list &lt;order_id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--type</h4>
<p>Limit result to customers or internal notes.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc order_note create &lt;order_id&gt;</h3>
<h4>--note</h4>
<p>Order note content. (<strong>Required</strong>)</p>
<h4>--customer_note</h4>
<p>Shows/define if the note is only for reference or for the customer (the user will be notified).</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc order_note get &lt;order_id&gt; &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc order_note delete &lt;order_id&gt; &lt;id&gt;</h3>
<h4>--force</h4>
<p>Required to be true, as resource does not support trashing.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc shop_order_refund</h2>
<h3>wc shop_order_refund list &lt;order_id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--page</h4>
<p>Current page of the collection.</p>
<h4>--per_page</h4>
<p>Maximum number of items to be returned in result set.</p>
<h4>--search</h4>
<p>Limit results to those matching a string.</p>
<h4>--after</h4>
<p>Limit response to resources published after a given ISO8601 compliant date.</p>
<h4>--before</h4>
<p>Limit response to resources published before a given ISO8601 compliant date.</p>
<h4>--exclude</h4>
<p>Ensure result set excludes specific IDs.</p>
<h4>--include</h4>
<p>Limit result set to specific ids.</p>
<h4>--offset</h4>
<p>Offset the result set by a specific number of items.</p>
<h4>--order</h4>
<p>Order sort attribute ascending or descending.</p>
<h4>--orderby</h4>
<p>Sort collection by object attribute.</p>
<h4>--filter</h4>
<p>Use WP Query arguments to modify the response; private query vars require appropriate authorization.</p>
<h4>--status</h4>
<p>Limit result set to orders assigned a specific status.</p>
<h4>--customer</h4>
<p>Limit result set to orders assigned a specific customer.</p>
<h4>--product</h4>
<p>Limit result set to orders assigned a specific product.</p>
<h4>--dp</h4>
<p>Number of decimal points to use in each resource.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc shop_order_refund create &lt;order_id&gt;</h3>
<h4>--amount</h4>
<p>Refund amount.</p>
<h4>--reason</h4>
<p>Reason for refund.</p>
<h4>--refunded_by</h4>
<p>User ID of user who created the refund.</p>
<h4>--meta_data</h4>
<p>Order meta data.</p>
<h4>--line_items</h4>
<p>Line items data.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc shop_order_refund get &lt;order_id&gt; &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc shop_order_refund delete &lt;order_id&gt; &lt;id&gt;</h3>
<h4>--force</h4>
<p>Required to be true, as resource does not support trashing.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc shop_order</h2>
<h3>wc shop_order list</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--page</h4>
<p>Current page of the collection.</p>
<h4>--per_page</h4>
<p>Maximum number of items to be returned in result set.</p>
<h4>--search</h4>
<p>Limit results to those matching a string.</p>
<h4>--after</h4>
<p>Limit response to resources published after a given ISO8601 compliant date.</p>
<h4>--before</h4>
<p>Limit response to resources published before a given ISO8601 compliant date.</p>
<h4>--exclude</h4>
<p>Ensure result set excludes specific IDs.</p>
<h4>--include</h4>
<p>Limit result set to specific ids.</p>
<h4>--offset</h4>
<p>Offset the result set by a specific number of items.</p>
<h4>--order</h4>
<p>Order sort attribute ascending or descending.</p>
<h4>--orderby</h4>
<p>Sort collection by object attribute.</p>
<h4>--filter</h4>
<p>Use WP Query arguments to modify the response; private query vars require appropriate authorization.</p>
<h4>--status</h4>
<p>Limit result set to orders assigned a specific status.</p>
<h4>--customer</h4>
<p>Limit result set to orders assigned a specific customer.</p>
<h4>--product</h4>
<p>Limit result set to orders assigned a specific product.</p>
<h4>--dp</h4>
<p>Number of decimal points to use in each resource.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc shop_order create</h3>
<h4>--parent_id</h4>
<p>Parent order ID.</p>
<h4>--status</h4>
<p>Order status.</p>
<h4>--currency</h4>
<p>Currency the order was created with, in ISO format.</p>
<h4>--date_created</h4>
<p>The date the order was created, in the site's timezone.</p>
<h4>--customer_id</h4>
<p>User ID who owns the order. 0 for guests.</p>
<h4>--billing</h4>
<p>Billing address.</p>
<h4>--shipping</h4>
<p>Shipping address.</p>
<h4>--payment_method</h4>
<p>Payment method ID.</p>
<h4>--payment_method_title</h4>
<p>Payment method title.</p>
<h4>--transaction_id</h4>
<p>Unique transaction ID.</p>
<h4>--customer_note</h4>
<p>Note left by customer during checkout.</p>
<h4>--date_completed</h4>
<p>The date the order was completed, in the site's timezone.</p>
<h4>--date_paid</h4>
<p>The date the order has been paid, in the site's timezone.</p>
<h4>--meta_data</h4>
<p>Order meta data.</p>
<h4>--line_items</h4>
<p>Line items data.</p>
<h4>--shipping_lines</h4>
<p>Shipping lines data.</p>
<h4>--fee_lines</h4>
<p>Fee lines data.</p>
<h4>--coupon_lines</h4>
<p>Coupons line data.</p>
<h4>--set_paid</h4>
<p>Define if the order is paid. It will set the status to processing and reduce stock items.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc shop_order get &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc shop_order update &lt;id&gt;</h3>
<h4>--parent_id</h4>
<p>Parent order ID.</p>
<h4>--status</h4>
<p>Order status.</p>
<h4>--currency</h4>
<p>Currency the order was created with, in ISO format.</p>
<h4>--date_created</h4>
<p>The date the order was created, in the site's timezone.</p>
<h4>--customer_id</h4>
<p>User ID who owns the order. 0 for guests.</p>
<h4>--billing</h4>
<p>Billing address.</p>
<h4>--shipping</h4>
<p>Shipping address.</p>
<h4>--payment_method</h4>
<p>Payment method ID.</p>
<h4>--payment_method_title</h4>
<p>Payment method title.</p>
<h4>--transaction_id</h4>
<p>Unique transaction ID.</p>
<h4>--customer_note</h4>
<p>Note left by customer during checkout.</p>
<h4>--date_completed</h4>
<p>The date the order was completed, in the site's timezone.</p>
<h4>--date_paid</h4>
<p>The date the order has been paid, in the site's timezone.</p>
<h4>--meta_data</h4>
<p>Order meta data.</p>
<h4>--line_items</h4>
<p>Line items data.</p>
<h4>--shipping_lines</h4>
<p>Shipping lines data.</p>
<h4>--fee_lines</h4>
<p>Fee lines data.</p>
<h4>--coupon_lines</h4>
<p>Coupons line data.</p>
<h4>--set_paid</h4>
<p>Define if the order is paid. It will set the status to processing and reduce stock items.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc shop_order delete &lt;id&gt;</h3>
<h4>--force</h4>
<p>Whether to bypass trash and force deletion.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc product_attribute_term</h2>
<h3>wc product_attribute_term list &lt;attribute_id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--page</h4>
<p>Current page of the collection.</p>
<h4>--per_page</h4>
<p>Maximum number of items to be returned in result set.</p>
<h4>--search</h4>
<p>Limit results to those matching a string.</p>
<h4>--exclude</h4>
<p>Ensure result set excludes specific ids.</p>
<h4>--include</h4>
<p>Limit result set to specific ids.</p>
<h4>--order</h4>
<p>Order sort attribute ascending or descending.</p>
<h4>--orderby</h4>
<p>Sort collection by resource attribute.</p>
<h4>--hide_empty</h4>
<p>Whether to hide resources not assigned to any products.</p>
<h4>--parent</h4>
<p>Limit result set to resources assigned to a specific parent.</p>
<h4>--product</h4>
<p>Limit result set to resources assigned to a specific product.</p>
<h4>--slug</h4>
<p>Limit result set to resources with a specific slug.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product_attribute_term create &lt;attribute_id&gt;</h3>
<h4>--name</h4>
<p>Name for the resource. (<strong>Required</strong>)</p>
<h4>--slug</h4>
<p>An alphanumeric identifier for the resource unique to its type.</p>
<h4>--description</h4>
<p>HTML description of the resource.</p>
<h4>--menu_order</h4>
<p>Menu order, used to custom sort the resource.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product_attribute_term get &lt;attribute_id&gt; &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product_attribute_term update &lt;attribute_id&gt; &lt;id&gt;</h3>
<h4>--name</h4>
<p>Term name.</p>
<h4>--slug</h4>
<p>An alphanumeric identifier for the resource unique to its type.</p>
<h4>--description</h4>
<p>HTML description of the resource.</p>
<h4>--menu_order</h4>
<p>Menu order, used to custom sort the resource.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product_attribute_term delete &lt;attribute_id&gt; &lt;id&gt;</h3>
<h4>--force</h4>
<p>Required to be true, as resource does not support trashing.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc product_attribute</h2>
<h3>wc product_attribute list</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product_attribute create</h3>
<h4>--name</h4>
<p>Name for the resource. (<strong>Required</strong>)</p>
<h4>--slug</h4>
<p>An alphanumeric identifier for the resource unique to its type.</p>
<h4>--type</h4>
<p>Type of attribute.</p>
<h4>--order_by</h4>
<p>Default sort order.</p>
<h4>--has_archives</h4>
<p>Enable/Disable attribute archives.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product_attribute get &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product_attribute update &lt;id&gt;</h3>
<h4>--name</h4>
<p>Attribute name.</p>
<h4>--slug</h4>
<p>An alphanumeric identifier for the resource unique to its type.</p>
<h4>--type</h4>
<p>Type of attribute.</p>
<h4>--order_by</h4>
<p>Default sort order.</p>
<h4>--has_archives</h4>
<p>Enable/Disable attribute archives.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product_attribute delete &lt;id&gt;</h3>
<h4>--force</h4>
<p>Required to be true, as resource does not support trashing.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc product_cat</h2>
<h3>wc product_cat list</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--page</h4>
<p>Current page of the collection.</p>
<h4>--per_page</h4>
<p>Maximum number of items to be returned in result set.</p>
<h4>--search</h4>
<p>Limit results to those matching a string.</p>
<h4>--exclude</h4>
<p>Ensure result set excludes specific ids.</p>
<h4>--include</h4>
<p>Limit result set to specific ids.</p>
<h4>--order</h4>
<p>Order sort attribute ascending or descending.</p>
<h4>--orderby</h4>
<p>Sort collection by resource attribute.</p>
<h4>--hide_empty</h4>
<p>Whether to hide resources not assigned to any products.</p>
<h4>--parent</h4>
<p>Limit result set to resources assigned to a specific parent.</p>
<h4>--product</h4>
<p>Limit result set to resources assigned to a specific product.</p>
<h4>--slug</h4>
<p>Limit result set to resources with a specific slug.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product_cat create</h3>
<h4>--name</h4>
<p>Name for the resource. (<strong>Required</strong>)</p>
<h4>--slug</h4>
<p>An alphanumeric identifier for the resource unique to its type.</p>
<h4>--parent</h4>
<p>The ID for the parent of the resource.</p>
<h4>--description</h4>
<p>HTML description of the resource.</p>
<h4>--display</h4>
<p>Category archive display type.</p>
<h4>--image</h4>
<p>Image data.</p>
<h4>--menu_order</h4>
<p>Menu order, used to custom sort the resource.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product_cat get &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product_cat update &lt;id&gt;</h3>
<h4>--name</h4>
<p>Category name.</p>
<h4>--slug</h4>
<p>An alphanumeric identifier for the resource unique to its type.</p>
<h4>--parent</h4>
<p>The ID for the parent of the resource.</p>
<h4>--description</h4>
<p>HTML description of the resource.</p>
<h4>--display</h4>
<p>Category archive display type.</p>
<h4>--image</h4>
<p>Image data.</p>
<h4>--menu_order</h4>
<p>Menu order, used to custom sort the resource.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product_cat delete &lt;id&gt;</h3>
<h4>--force</h4>
<p>Required to be true, as resource does not support trashing.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc product_review</h2>
<h3>wc product_review list &lt;product_id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product_review create &lt;product_id&gt;</h3>
<h4>--review</h4>
<p>Review content. (<strong>Required</strong>)</p>
<h4>--date_created</h4>
<p>The date the review was created, in the site's timezone.</p>
<h4>--rating</h4>
<p>Review rating (0 to 5).</p>
<h4>--name</h4>
<p>Name of the reviewer. (<strong>Required</strong>)</p>
<h4>--email</h4>
<p>Email of the reviewer. (<strong>Required</strong>)</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product_review get &lt;product_id&gt; &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product_review update &lt;product_id&gt; &lt;id&gt;</h3>
<h4>--review</h4>
<p>The content of the review.</p>
<h4>--date_created</h4>
<p>The date the review was created, in the site's timezone.</p>
<h4>--rating</h4>
<p>Review rating (0 to 5).</p>
<h4>--name</h4>
<p>Reviewer name.</p>
<h4>--email</h4>
<p>Reviewer email.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product_review delete &lt;product_id&gt; &lt;id&gt;</h3>
<h4>--force</h4>
<p>Whether to bypass trash and force deletion.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc product_shipping_class</h2>
<h3>wc product_shipping_class list</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--page</h4>
<p>Current page of the collection.</p>
<h4>--per_page</h4>
<p>Maximum number of items to be returned in result set.</p>
<h4>--search</h4>
<p>Limit results to those matching a string.</p>
<h4>--exclude</h4>
<p>Ensure result set excludes specific ids.</p>
<h4>--include</h4>
<p>Limit result set to specific ids.</p>
<h4>--offset</h4>
<p>Offset the result set by a specific number of items.</p>
<h4>--order</h4>
<p>Order sort attribute ascending or descending.</p>
<h4>--orderby</h4>
<p>Sort collection by resource attribute.</p>
<h4>--hide_empty</h4>
<p>Whether to hide resources not assigned to any products.</p>
<h4>--product</h4>
<p>Limit result set to resources assigned to a specific product.</p>
<h4>--slug</h4>
<p>Limit result set to resources with a specific slug.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product_shipping_class create</h3>
<h4>--name</h4>
<p>Name for the resource. (<strong>Required</strong>)</p>
<h4>--slug</h4>
<p>An alphanumeric identifier for the resource unique to its type.</p>
<h4>--description</h4>
<p>HTML description of the resource.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product_shipping_class get &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product_shipping_class update &lt;id&gt;</h3>
<h4>--name</h4>
<p>Shipping class name.</p>
<h4>--slug</h4>
<p>An alphanumeric identifier for the resource unique to its type.</p>
<h4>--description</h4>
<p>HTML description of the resource.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product_shipping_class delete &lt;id&gt;</h3>
<h4>--force</h4>
<p>Required to be true, as resource does not support trashing.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc product_tag</h2>
<h3>wc product_tag list</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--page</h4>
<p>Current page of the collection.</p>
<h4>--per_page</h4>
<p>Maximum number of items to be returned in result set.</p>
<h4>--search</h4>
<p>Limit results to those matching a string.</p>
<h4>--exclude</h4>
<p>Ensure result set excludes specific ids.</p>
<h4>--include</h4>
<p>Limit result set to specific ids.</p>
<h4>--offset</h4>
<p>Offset the result set by a specific number of items.</p>
<h4>--order</h4>
<p>Order sort attribute ascending or descending.</p>
<h4>--orderby</h4>
<p>Sort collection by resource attribute.</p>
<h4>--hide_empty</h4>
<p>Whether to hide resources not assigned to any products.</p>
<h4>--product</h4>
<p>Limit result set to resources assigned to a specific product.</p>
<h4>--slug</h4>
<p>Limit result set to resources with a specific slug.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product_tag create</h3>
<h4>--name</h4>
<p>Name for the resource. (<strong>Required</strong>)</p>
<h4>--slug</h4>
<p>An alphanumeric identifier for the resource unique to its type.</p>
<h4>--description</h4>
<p>HTML description of the resource.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product_tag get &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product_tag update &lt;id&gt;</h3>
<h4>--name</h4>
<p>Tag name.</p>
<h4>--slug</h4>
<p>An alphanumeric identifier for the resource unique to its type.</p>
<h4>--description</h4>
<p>HTML description of the resource.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product_tag delete &lt;id&gt;</h3>
<h4>--force</h4>
<p>Required to be true, as resource does not support trashing.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc product</h2>
<h3>wc product list</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--page</h4>
<p>Current page of the collection.</p>
<h4>--per_page</h4>
<p>Maximum number of items to be returned in result set.</p>
<h4>--search</h4>
<p>Limit results to those matching a string.</p>
<h4>--after</h4>
<p>Limit response to resources published after a given ISO8601 compliant date.</p>
<h4>--before</h4>
<p>Limit response to resources published before a given ISO8601 compliant date.</p>
<h4>--exclude</h4>
<p>Ensure result set excludes specific IDs.</p>
<h4>--include</h4>
<p>Limit result set to specific ids.</p>
<h4>--offset</h4>
<p>Offset the result set by a specific number of items.</p>
<h4>--order</h4>
<p>Order sort attribute ascending or descending.</p>
<h4>--orderby</h4>
<p>Sort collection by object attribute.</p>
<h4>--filter</h4>
<p>Use WP Query arguments to modify the response; private query vars require appropriate authorization.</p>
<h4>--slug</h4>
<p>Limit result set to products with a specific slug.</p>
<h4>--status</h4>
<p>Limit result set to products assigned a specific status.</p>
<h4>--type</h4>
<p>Limit result set to products assigned a specific type.</p>
<h4>--sku</h4>
<p>Limit result set to products with a specific SKU.</p>
<h4>--featured</h4>
<p>Limit result set to featured products.</p>
<h4>--category</h4>
<p>Limit result set to products assigned a specific category ID.</p>
<h4>--tag</h4>
<p>Limit result set to products assigned a specific tag ID.</p>
<h4>--shipping_class</h4>
<p>Limit result set to products assigned a specific shipping class ID.</p>
<h4>--attribute</h4>
<p>Limit result set to products with a specific attribute.</p>
<h4>--attribute_term</h4>
<p>Limit result set to products with a specific attribute term ID (required an assigned attribute).</p>
<h4>--tax_class</h4>
<p>Limit result set to products with a specific tax class.</p>
<h4>--in_stock</h4>
<p>Limit result set to products in stock or out of stock.</p>
<h4>--on_sale</h4>
<p>Limit result set to products on sale.</p>
<h4>--min_price</h4>
<p>Limit result set to products based on a minimum price.</p>
<h4>--max_price</h4>
<p>Limit result set to products based on a maximum price.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product create</h3>
<h4>--name</h4>
<p>Product name.</p>
<h4>--slug</h4>
<p>Product slug.</p>
<h4>--type</h4>
<p>Product type.</p>
<h4>--status</h4>
<p>Product status (post status).</p>
<h4>--featured</h4>
<p>Featured product.</p>
<h4>--catalog_visibility</h4>
<p>Catalog visibility.</p>
<h4>--description</h4>
<p>Product description.</p>
<h4>--short_description</h4>
<p>Product short description.</p>
<h4>--sku</h4>
<p>Unique identifier.</p>
<h4>--regular_price</h4>
<p>Product regular price.</p>
<h4>--sale_price</h4>
<p>Product sale price.</p>
<h4>--date_on_sale_from</h4>
<p>Start date of sale price.</p>
<h4>--date_on_sale_to</h4>
<p>End data of sale price.</p>
<h4>--virtual</h4>
<p>If the product is virtual.</p>
<h4>--downloadable</h4>
<p>If the product is downloadable.</p>
<h4>--downloads</h4>
<p>List of downloadable files.</p>
<h4>--download_limit</h4>
<p>Amount of times the product can be downloaded.</p>
<h4>--download_expiry</h4>
<p>Number of days that the customer has up to be able to download the product.</p>
<h4>--download_type</h4>
<p>Download type, this controls the schema on the front-end.</p>
<h4>--external_url</h4>
<p>Product external URL. Only for external products.</p>
<h4>--button_text</h4>
<p>Product external button text. Only for external products.</p>
<h4>--tax_status</h4>
<p>Tax status.</p>
<h4>--tax_class</h4>
<p>Tax class.</p>
<h4>--manage_stock</h4>
<p>Stock management at product level.</p>
<h4>--stock_quantity</h4>
<p>Stock quantity.</p>
<h4>--in_stock</h4>
<p>Controls whether or not the product is listed as "in stock" or "out of stock" on the frontend.</p>
<h4>--backorders</h4>
<p>If managing stock, this controls if backorders are allowed.</p>
<h4>--sold_individually</h4>
<p>Allow one item to be bought in a single order.</p>
<h4>--weight</h4>
<p>Product weight (lbs).</p>
<h4>--dimensions</h4>
<p>Product dimensions.</p>
<h4>--shipping_class</h4>
<p>Shipping class slug.</p>
<h4>--reviews_allowed</h4>
<p>Allow reviews.</p>
<h4>--upsell_ids</h4>
<p>List of up-sell products IDs.</p>
<h4>--cross_sell_ids</h4>
<p>List of cross-sell products IDs.</p>
<h4>--parent_id</h4>
<p>Product parent ID.</p>
<h4>--purchase_note</h4>
<p>Optional note to send the customer after purchase.</p>
<h4>--categories</h4>
<p>List of categories.</p>
<h4>--tags</h4>
<p>List of tags.</p>
<h4>--images</h4>
<p>List of images.</p>
<h4>--attributes</h4>
<p>List of attributes.</p>
<h4>--default_attributes</h4>
<p>Defaults variation attributes.</p>
<h4>--variations</h4>
<p>List of variations.</p>
<h4>--menu_order</h4>
<p>Menu order, used to custom sort products.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product get &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product update &lt;id&gt;</h3>
<h4>--name</h4>
<p>Product name.</p>
<h4>--slug</h4>
<p>Product slug.</p>
<h4>--type</h4>
<p>Product type.</p>
<h4>--status</h4>
<p>Product status (post status).</p>
<h4>--featured</h4>
<p>Featured product.</p>
<h4>--catalog_visibility</h4>
<p>Catalog visibility.</p>
<h4>--description</h4>
<p>Product description.</p>
<h4>--short_description</h4>
<p>Product short description.</p>
<h4>--sku</h4>
<p>Unique identifier.</p>
<h4>--regular_price</h4>
<p>Product regular price.</p>
<h4>--sale_price</h4>
<p>Product sale price.</p>
<h4>--date_on_sale_from</h4>
<p>Start date of sale price.</p>
<h4>--date_on_sale_to</h4>
<p>End data of sale price.</p>
<h4>--virtual</h4>
<p>If the product is virtual.</p>
<h4>--downloadable</h4>
<p>If the product is downloadable.</p>
<h4>--downloads</h4>
<p>List of downloadable files.</p>
<h4>--download_limit</h4>
<p>Amount of times the product can be downloaded.</p>
<h4>--download_expiry</h4>
<p>Number of days that the customer has up to be able to download the product.</p>
<h4>--download_type</h4>
<p>Download type, this controls the schema on the front-end.</p>
<h4>--external_url</h4>
<p>Product external URL. Only for external products.</p>
<h4>--button_text</h4>
<p>Product external button text. Only for external products.</p>
<h4>--tax_status</h4>
<p>Tax status.</p>
<h4>--tax_class</h4>
<p>Tax class.</p>
<h4>--manage_stock</h4>
<p>Stock management at product level.</p>
<h4>--stock_quantity</h4>
<p>Stock quantity.</p>
<h4>--in_stock</h4>
<p>Controls whether or not the product is listed as "in stock" or "out of stock" on the frontend.</p>
<h4>--backorders</h4>
<p>If managing stock, this controls if backorders are allowed.</p>
<h4>--sold_individually</h4>
<p>Allow one item to be bought in a single order.</p>
<h4>--weight</h4>
<p>Product weight (lbs).</p>
<h4>--dimensions</h4>
<p>Product dimensions.</p>
<h4>--shipping_class</h4>
<p>Shipping class slug.</p>
<h4>--reviews_allowed</h4>
<p>Allow reviews.</p>
<h4>--upsell_ids</h4>
<p>List of up-sell products IDs.</p>
<h4>--cross_sell_ids</h4>
<p>List of cross-sell products IDs.</p>
<h4>--parent_id</h4>
<p>Product parent ID.</p>
<h4>--purchase_note</h4>
<p>Optional note to send the customer after purchase.</p>
<h4>--categories</h4>
<p>List of categories.</p>
<h4>--tags</h4>
<p>List of tags.</p>
<h4>--images</h4>
<p>List of images.</p>
<h4>--attributes</h4>
<p>List of attributes.</p>
<h4>--default_attributes</h4>
<p>Defaults variation attributes.</p>
<h4>--variations</h4>
<p>List of variations.</p>
<h4>--menu_order</h4>
<p>Menu order, used to custom sort products.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product delete &lt;id&gt;</h3>
<h4>--force</h4>
<p>Whether to bypass trash and force deletion.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc product_variation</h2>
<h3>wc product_variation list &lt;product_id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--page</h4>
<p>Current page of the collection.</p>
<h4>--per_page</h4>
<p>Maximum number of items to be returned in result set.</p>
<h4>--search</h4>
<p>Limit results to those matching a string.</p>
<h4>--after</h4>
<p>Limit response to resources published after a given ISO8601 compliant date.</p>
<h4>--before</h4>
<p>Limit response to resources published before a given ISO8601 compliant date.</p>
<h4>--exclude</h4>
<p>Ensure result set excludes specific IDs.</p>
<h4>--include</h4>
<p>Limit result set to specific ids.</p>
<h4>--offset</h4>
<p>Offset the result set by a specific number of items.</p>
<h4>--order</h4>
<p>Order sort attribute ascending or descending.</p>
<h4>--orderby</h4>
<p>Sort collection by object attribute.</p>
<h4>--filter</h4>
<p>Use WP Query arguments to modify the response; private query vars require appropriate authorization.</p>
<h4>--slug</h4>
<p>Limit result set to products with a specific slug.</p>
<h4>--status</h4>
<p>Limit result set to products assigned a specific status.</p>
<h4>--type</h4>
<p>Limit result set to products assigned a specific type.</p>
<h4>--sku</h4>
<p>Limit result set to products with a specific SKU.</p>
<h4>--featured</h4>
<p>Limit result set to featured products.</p>
<h4>--category</h4>
<p>Limit result set to products assigned a specific category ID.</p>
<h4>--tag</h4>
<p>Limit result set to products assigned a specific tag ID.</p>
<h4>--shipping_class</h4>
<p>Limit result set to products assigned a specific shipping class ID.</p>
<h4>--attribute</h4>
<p>Limit result set to products with a specific attribute.</p>
<h4>--attribute_term</h4>
<p>Limit result set to products with a specific attribute term ID (required an assigned attribute).</p>
<h4>--tax_class</h4>
<p>Limit result set to products with a specific tax class.</p>
<h4>--in_stock</h4>
<p>Limit result set to products in stock or out of stock.</p>
<h4>--on_sale</h4>
<p>Limit result set to products on sale.</p>
<h4>--min_price</h4>
<p>Limit result set to products based on a minimum price.</p>
<h4>--max_price</h4>
<p>Limit result set to products based on a maximum price.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product_variation create &lt;product_id&gt;</h3>
<h4>--description</h4>
<p>Variation description.</p>
<h4>--sku</h4>
<p>Unique identifier.</p>
<h4>--regular_price</h4>
<p>Variation regular price.</p>
<h4>--sale_price</h4>
<p>Variation sale price.</p>
<h4>--date_on_sale_from</h4>
<p>Start date of sale price.</p>
<h4>--date_on_sale_to</h4>
<p>End data of sale price.</p>
<h4>--visible</h4>
<p>Define if the attribute is visible on the "Additional information" tab in the product's page.</p>
<h4>--virtual</h4>
<p>If the variation is virtual.</p>
<h4>--downloadable</h4>
<p>If the variation is downloadable.</p>
<h4>--downloads</h4>
<p>List of downloadable files.</p>
<h4>--download_limit</h4>
<p>Amount of times the variation can be downloaded.</p>
<h4>--download_expiry</h4>
<p>Number of days that the customer has up to be able to download the variation.</p>
<h4>--tax_status</h4>
<p>Tax status.</p>
<h4>--tax_class</h4>
<p>Tax class.</p>
<h4>--manage_stock</h4>
<p>Stock management at variation level.</p>
<h4>--stock_quantity</h4>
<p>Stock quantity.</p>
<h4>--in_stock</h4>
<p>Controls whether or not the variation is listed as "in stock" or "out of stock" on the frontend.</p>
<h4>--backorders</h4>
<p>If managing stock, this controls if backorders are allowed.</p>
<h4>--weight</h4>
<p>Variation weight (lbs).</p>
<h4>--dimensions</h4>
<p>Variation dimensions.</p>
<h4>--shipping_class</h4>
<p>Shipping class slug.</p>
<h4>--image</h4>
<p>Variation image data.</p>
<h4>--attributes</h4>
<p>List of attributes.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product_variation get &lt;product_id&gt; &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc product_variation update &lt;product_id&gt; &lt;id&gt;</h3>
<h4>--description</h4>
<p>Variation description.</p>
<h4>--sku</h4>
<p>Unique identifier.</p>
<h4>--regular_price</h4>
<p>Variation regular price.</p>
<h4>--sale_price</h4>
<p>Variation sale price.</p>
<h4>--date_on_sale_from</h4>
<p>Start date of sale price.</p>
<h4>--date_on_sale_to</h4>
<p>End data of sale price.</p>
<h4>--visible</h4>
<p>Define if the attribute is visible on the "Additional information" tab in the product's page.</p>
<h4>--virtual</h4>
<p>If the variation is virtual.</p>
<h4>--downloadable</h4>
<p>If the variation is downloadable.</p>
<h4>--downloads</h4>
<p>List of downloadable files.</p>
<h4>--download_limit</h4>
<p>Amount of times the variation can be downloaded.</p>
<h4>--download_expiry</h4>
<p>Number of days that the customer has up to be able to download the variation.</p>
<h4>--tax_status</h4>
<p>Tax status.</p>
<h4>--tax_class</h4>
<p>Tax class.</p>
<h4>--manage_stock</h4>
<p>Stock management at variation level.</p>
<h4>--stock_quantity</h4>
<p>Stock quantity.</p>
<h4>--in_stock</h4>
<p>Controls whether or not the variation is listed as "in stock" or "out of stock" on the frontend.</p>
<h4>--backorders</h4>
<p>If managing stock, this controls if backorders are allowed.</p>
<h4>--weight</h4>
<p>Variation weight (lbs).</p>
<h4>--dimensions</h4>
<p>Variation dimensions.</p>
<h4>--shipping_class</h4>
<p>Shipping class slug.</p>
<h4>--image</h4>
<p>Variation image data.</p>
<h4>--attributes</h4>
<p>List of attributes.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc product_variation delete &lt;product_id&gt; &lt;id&gt;</h3>
<h4>--force</h4>
<p>Whether to bypass trash and force deletion.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc shipping_zone</h2>
<h3>wc shipping_zone list</h3>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc shipping_zone create</h3>
<h4>--name</h4>
<p>Shipping zone name. (<strong>Required</strong>)</p>
<h4>--order</h4>
<p>Shipping zone order.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc shipping_zone get &lt;id&gt;</h3>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc shipping_zone update &lt;id&gt;</h3>
<h4>--name</h4>
<p>Shipping zone name.</p>
<h4>--order</h4>
<p>Shipping zone order.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc shipping_zone delete &lt;id&gt;</h3>
<h4>--force</h4>
<p>Whether to bypass trash and force deletion.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc shipping_zone_location</h2>
<h3>wc shipping_zone_location list</h3>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h2>wc shipping_zone_method</h2>
<h3>wc shipping_zone_method list</h3>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc shipping_zone_method create</h3>
<h4>--order</h4>
<p>Shipping method sort order.</p>
<h4>--enabled</h4>
<p>Shipping method enabled status.</p>
<h4>--method_id</h4>
<p>Shipping method ID. Write on create only.</p>
<h4>--settings</h4>
<p>Shipping method settings.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc shipping_zone_method get &lt;id&gt;</h3>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc shipping_zone_method update &lt;id&gt;</h3>
<h4>--order</h4>
<p>Shipping method sort order.</p>
<h4>--enabled</h4>
<p>Shipping method enabled status.</p>
<h4>--method_id</h4>
<p>Shipping method ID. Write on create only.</p>
<h4>--settings</h4>
<p>Shipping method settings.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc shipping_zone_method delete &lt;id&gt;</h3>
<h4>--force</h4>
<p>Whether to bypass trash and force deletion.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc tax_class</h2>
<h3>wc tax_class list</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc tax_class create</h3>
<h4>--name</h4>
<p>Tax class name. (<strong>Required</strong>)</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc tax_class delete &lt;id&gt;</h3>
<h4>--force</h4>
<p>Required to be true, as resource does not support trashing.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc tax</h2>
<h3>wc tax list</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--page</h4>
<p>Current page of the collection.</p>
<h4>--per_page</h4>
<p>Maximum number of items to be returned in result set.</p>
<h4>--search</h4>
<p>Limit results to those matching a string.</p>
<h4>--exclude</h4>
<p>Ensure result set excludes specific IDs.</p>
<h4>--include</h4>
<p>Limit result set to specific IDs.</p>
<h4>--offset</h4>
<p>Offset the result set by a specific number of items.</p>
<h4>--order</h4>
<p>Order sort attribute ascending or descending.</p>
<h4>--orderby</h4>
<p>Sort collection by object attribute.</p>
<h4>--class</h4>
<p>Sort by tax class.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc tax create</h3>
<h4>--country</h4>
<p>Country ISO 3166 code.</p>
<h4>--state</h4>
<p>State code.</p>
<h4>--postcode</h4>
<p>Postcode / ZIP.</p>
<h4>--city</h4>
<p>City name.</p>
<h4>--rate</h4>
<p>Tax rate.</p>
<h4>--name</h4>
<p>Tax rate name.</p>
<h4>--priority</h4>
<p>Tax priority.</p>
<h4>--compound</h4>
<p>Whether or not this is a compound rate.</p>
<h4>--shipping</h4>
<p>Whether or not this tax rate also gets applied to shipping.</p>
<h4>--order</h4>
<p>Indicates the order that will appear in queries.</p>
<h4>--class</h4>
<p>Tax class.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc tax get &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc tax update &lt;id&gt;</h3>
<h4>--country</h4>
<p>Country ISO 3166 code.</p>
<h4>--state</h4>
<p>State code.</p>
<h4>--postcode</h4>
<p>Postcode / ZIP.</p>
<h4>--city</h4>
<p>City name.</p>
<h4>--rate</h4>
<p>Tax rate.</p>
<h4>--name</h4>
<p>Tax rate name.</p>
<h4>--priority</h4>
<p>Tax priority.</p>
<h4>--compound</h4>
<p>Whether or not this is a compound rate.</p>
<h4>--shipping</h4>
<p>Whether or not this tax rate also gets applied to shipping.</p>
<h4>--order</h4>
<p>Indicates the order that will appear in queries.</p>
<h4>--class</h4>
<p>Tax class.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc tax delete &lt;id&gt;</h3>
<h4>--force</h4>
<p>Required to be true, as resource does not support trashing.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc webhook_delivery</h2>
<h3>wc webhook_delivery list</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc webhook_delivery get &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h2>wc webhook</h2>
<h3>wc webhook list</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--page</h4>
<p>Current page of the collection.</p>
<h4>--per_page</h4>
<p>Maximum number of items to be returned in result set.</p>
<h4>--search</h4>
<p>Limit results to those matching a string.</p>
<h4>--after</h4>
<p>Limit response to resources published after a given ISO8601 compliant date.</p>
<h4>--before</h4>
<p>Limit response to resources published before a given ISO8601 compliant date.</p>
<h4>--exclude</h4>
<p>Ensure result set excludes specific IDs.</p>
<h4>--include</h4>
<p>Limit result set to specific ids.</p>
<h4>--offset</h4>
<p>Offset the result set by a specific number of items.</p>
<h4>--order</h4>
<p>Order sort attribute ascending or descending.</p>
<h4>--orderby</h4>
<p>Sort collection by object attribute.</p>
<h4>--filter</h4>
<p>Use WP Query arguments to modify the response; private query vars require appropriate authorization.</p>
<h4>--status</h4>
<p>Limit result set to webhooks assigned a specific status.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc webhook create</h3>
<h4>--name</h4>
<p>A friendly name for the webhook.</p>
<h4>--status</h4>
<p>Webhook status.</p>
<h4>--topic</h4>
<p>Webhook topic. (<strong>Required</strong>)</p>
<h4>--secret</h4>
<p>Webhook secret. (<strong>Required</strong>)</p>
<h4>--delivery_url</h4>
<p>Webhook delivery URL. (<strong>Required</strong>)</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc webhook get &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc webhook update &lt;id&gt;</h3>
<h4>--name</h4>
<p>A friendly name for the webhook.</p>
<h4>--status</h4>
<p>Webhook status.</p>
<h4>--topic</h4>
<p>Webhook topic.</p>
<h4>--secret</h4>
<p>Secret key used to generate a hash of the delivered webhook and provided in the request headers. This will default is a MD5 hash from the current user's ID|username if not provided.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h3>wc webhook delete &lt;id&gt;</h3>
<h4>--force</h4>
<p>Required to be true, as resource does not support trashing.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
<h2>wc shipping_method</h2>
<h3>wc shipping_method list</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc shipping_method get &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h2>wc payment_gateway</h2>
<h3>wc payment_gateway list</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc payment_gateway get &lt;id&gt;</h3>
<h4>--context</h4>
<p>Scope under which the request is made; determines fields present in response.</p>
<h4>--fields</h4>
<p>Limit response to specific fields. Defaults to all fields.</p>
<h4>--field</h4>
<p>Get the value of an individual field.</p>
<h4>--format</h4>
<p>Render response in a particular format.<br>Default: table<br />Options: table, json, csv, ids, yaml, count, headers, body, envelope</p>
<h3>wc payment_gateway update &lt;id&gt;</h3>
<h4>--title</h4>
<p>Payment gateway title on checkout.</p>
<h4>--description</h4>
<p>Payment gateway description on checkout.</p>
<h4>--order</h4>
<p>Payment gateway sort order.</p>
<h4>--enabled</h4>
<p>Payment gateway enabled status.</p>
<h4>--settings</h4>
<p>Payment gateway settings.</p>
<h4>--porcelain</h4>
<p>Output just the id when the operation is successful.</p>
